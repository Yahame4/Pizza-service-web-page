...
class Kunde extends Page
{
    ...
    protected function generateView():void
    {
        ...
        echo "<h2>Ihre Bestellungen:</h2>";

        if (empty($data)) {
            echo "Sie haben momentan keine Bestellungen oder ihre Bestellungen wurden bereits ausgeliefert!";
        } else {
            foreach($data as $pizza) {
                $pizzaName = htmlspecialchars($pizza[0]);
                $ordering_id = htmlspecialchars($pizza[2]);
                
                echo <<<HTML
                <p>{$pizzaName}: {$this->getStatus($pizza[1])}</p>
                HTML;
            }
        }      
