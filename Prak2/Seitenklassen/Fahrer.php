    ...
    protected function generateView():void
    {
        ...
        if (empty($data)) {
            ...
            foreach($data as $order) {
                ...
                $totalPrice = "45,56 €";
                $orderPizzalist = $this->splitPizzaList($order[2]);

                echo <<<HTML
                <form action="./Fahrer.php" method="POST" id="form{$order[0]}">
                  <h3>{$orderAddress}</h3>
                  <h4>{$orderPizzalist}  $totalPrice</h4>
                  <input type="radio" name="lieferstatus" id="form{$orderid}fertig" value="2"{$this->isChecked($order[3], 2)}>
                  <label for="form{$orderid}fertig">fertig</label>
                  <input type="radio" name="lieferstatus" id="form{$orderid}unterwegs" value="3"{$this->isChecked($order[3], 3)}>
                  <label for="form{$orderid}unterwegs">unterwegs</label>
                  <input type="radio" name="lieferstatus" id="form{$orderid}geliefert" value="4"{$this->isChecked($order[3], 4)}>
