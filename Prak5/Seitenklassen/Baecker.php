...
class Baecker extends Page
{
    ...
    protected function generateView():void
    {
        ...
        if (empty($data)) {
            echo <<<HTML
            <p class="pBacker"><b> Es gibt nix zu tun! Es liegen keine Bestellungen mehr vor!</b></p>
            <div class="Seperator"></div>
            HTML;
        } else {
        ...
        foreach($data as $pizza) {
            ...
            echo <<<HTML
                ...
                <p class="pBacker"><b>#{$pizza_Id} - Pizza {$pizza_Name}:</b></p>
                <input type="radio" name="status" id="form{$pizza_Id}bestellt" value="0" onclick="document.forms['form{$pizza_Id}'].submit();" {$this->Checked($pizza_Status,0)}> 
                <label class="textOrdered" for="form{$pizza_Id}bestellt">bestellt</label>
                <input type="radio" name="status" id="form{$pizza_Id}imOfen" value="1" onclick="document.forms['form{$pizza_Id}'].submit();" {$this->Checked($pizza_Status,1)}> 
                <label class="textOven" for="form{$pizza_Id}imOfen">im Ofen</label>
                <input type="radio" name="status" id="form{$pizza_Id}fertig" value="2" onclick="document.forms['form{$pizza_Id}'].submit();" {$this->Checked($pizza_Status,2)}> 
                <label class="textFinished" for="form{$pizza_Id}fertig">fertig</label>
                <input type="hidden" name="action" value="{$pizza_Id}" tabindex="1" accesskey="s">
                </form> <br>
                <div class="Seperator"></div>
            HTML;
            }
